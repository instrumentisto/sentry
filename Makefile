...
SENTRY_VER ?= $(strip \
	$(shell grep 'ARG sentry_ver=' Dockerfile | cut -d '=' -f2))
SENTRY_TELEGRAM_VER ?= $(strip \
	$(shell grep 'ARG sentry_telegram_ver=' Dockerfile | cut -d '=' -f2))

IMAGE_NAME := instrumentisto/sentry
TAGS ?= 1.0.0-sentry$(SENTRY_VER)-sentry_telegram$(SENTRY_TELEGRAM_VER) \
        1.0.0 \
        1.0 \
        latest
...
#	           [SENTRY_VER=<sentry-version>]
#	           [SENTRY_TELEGRAM_VER=<sentry_telegram-version>]
...
image:
	docker build --network=host --force-rm \
		...
		--build-arg sentry_ver=$(SENTRY_VER) \
		--build-arg sentry_telegram_ver=$(SENTRY_TELEGRAM_VER) \
...
release:
	@make image tag=$(tag) no-cache=$(no-cache) \
	            SENTRY_VER=$(SENTRY_VER) \
	            SENTRY_TELEGRAM_VER=$(SENTRY_TELEGRAM_VER)
...
post-push-hook:
	@mkdir -p hooks/
	docker run --rm -v "C:\\Repositories\\sentry-with-telegram/post_push.tmpl.php":/post_push.php:ro \
